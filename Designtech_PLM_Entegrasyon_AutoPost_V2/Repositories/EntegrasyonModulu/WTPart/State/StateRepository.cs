...
namespace Designtech_PLM_Entegrasyon_AutoPost_V2.Repositories.EntegrasyonModulu.WTPart.State
{
	public class StateRepository : IStateService
	{
			...
			try
			{
				...
				while (hasMoreRecords)
				{
					...
					foreach (var partItem in resolvedItemsList)
						...
						await ProcessResponse(response, state, conn, configuration, apiFullUrl, apiURL, endPoint, partItem.ParcaPartID);
		...
		private async Task ProcessResponse(Part response, string state, IDbConnection conn, IConfiguration configuration, string apiFullUrl, string apiURL, string endPoint,long ID)
		{
