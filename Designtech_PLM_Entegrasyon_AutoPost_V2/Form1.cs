...
CREATE TRIGGER [{scheman}].[Part_Document_ONR]
...
BEGIN
    ...
    -- En büyük DocMaster_ID deðerini ve PartID'yi al
    ...
    -- Son versiyon bilgilerini al (PartID kullanýlarak)
    SELECT TOP 1 
        @SonVersiyonHarf = WT_FBI_COMPUTE_3_1, 
        @SonVersiyonNo = WT_FBI_COMPUTE_3_2,
        @SonVersiyonPartID = idA2A2
    FROM [{scheman}].WTPart
    WHERE idA2A2 = @PartID; -- @idA2A2 yerine @PartID kullanýldý
    ...
    -- Eski versiyonlarda olup olmadýðýný kontrol et (idA3masterReference kullanarak)
    IF NOT EXISTS (
        SELECT 1
    ...
    BEGIN
        ...
        -- DocMaster_IDGecici deðeri ile eþleþen tek bir kayýt olup olmadýðýný kontrol et
        IF (SELECT COUNT(*) FROM [{scheman}].[WTPartReferenceLink] WHERE idA3B5 = @DocMaster_IDGecici) = 1
        BEGIN
            -- Tek bir kayýt varsa, bilgileri al
            SELECT @idA3A5 = idA3A5, @idA3B5 = idA3B5, @idA2A2 = idA2A2 
            FROM [{scheman}].[WTPartReferenceLink] 
            WHERE idA3B5 = @DocMaster_IDGecici;

            -- Toplam veri sayýsýný al 
...
END
    ...
    IF @referenceType = 'DRAWING' and @empResult = 0 
    BEGIN
        SELECT 
        ...
        ELSE
        BEGIN
                SET @newDocumentNumber = @documentNumber  + @contentName;
        ...
        END
            ...
            ELSE
            BEGIN
                SET @newDocumentNumber = @contentName  + @documentNumber;
            END
        END
        ELSE
        BEGIN
        ...
        -- Toplam sayýsýný hesaplýyoruz
        SELECT @toplamSayi = COUNT(*)
        FROM [{scheman}].Des_CadDocumentBaglaLog
        WHERE mainIdA2A2 = @idA3B5;

        SET @newDocumentNumber = @newDocumentNumber + '_'  + CAST(@toplamSayi + 1 AS NVARCHAR(50));

        SET @newName = @name  + @contentName  + CAST(@toplamSayi + 1 AS NVARCHAR(50));
...
";
		...
		private async Task ProcessStateAsync(string state, string catalogValue, SqlConnection conn, string apiFullUrl, string apiURL, string CSRF_NONCE, string WindchillServerName, string ServerName, string BasicUsername, string BasicPassword, DateTime anlikTarih, string sourceApi, string endPoint, int oldAlternateLinkCount, string sablonDataDurumu, string API_ENDPOINT_ALTERNATE_PART, string API_ENDPOINT_REMOVED, string API_ENDPOINT_SEND_FILE)
		{
			...
			if (sourceApi.Contains("CADDocumentMgmt"))
			{
				...
				{
					// Saat kontrolü
					if (now.Hour >= 20 && now.Hour < 23 && now.Minute <= 59)
					{
						var errorSql = $"SELECT [Ent_ID], [EPMDocID], [StateDegeri],[idA3masterReference] FROM {catalogValue}.Ent_EPMDocState_ERROR WHERE [StateDegeri] = 'RELEASED'";

						var errorRecords = await conn.QueryAsync<Ent_EPMDocStateModel>(errorSql);

						if (errorRecords.Any())
						{
							// Eðer errorRecords boþ deðilse
							sql = errorSql;
						}
						else
						{
							// Eðer errorRecords boþsa
							sql = $"SELECT [Ent_ID], [EPMDocID], [StateDegeri],[idA3masterReference] FROM {catalogValue}.Ent_EPMDocState WHERE [StateDegeri] = 'RELEASED'";
						}
					}
					else
					{
						// Saat aralýðý dýþýnda
						sql = $"SELECT [Ent_ID], [EPMDocID], [StateDegeri],[idA3masterReference] FROM {catalogValue}.Ent_EPMDocState WHERE [StateDegeri] = 'RELEASED'";
					}
					//sql = $"SELECT [Ent_ID], [EPMDocID], [StateDegeri] FROM {catalogValue}.Ent_EPMDocState_ERROR WHERE [StateDegeri] = 'RELEASED'";
				}
				if (state == "CANCELLED")
				{
					if (now.Hour >= 20 && now.Hour < 23 && now.Minute <= 59)
					{
						...
						var errorSql = $"SELECT [Ent_ID], [EPMDocID], [StateDegeri],[idA3masterReference] FROM {catalogValue}.Ent_EPMDocState_CANCELLED_ERROR WHERE [StateDegeri] = 'CANCELLED'";
						...
						{
							// Eðer errorRecords boþsa
							sql = $"SELECT [Ent_ID], [EPMDocID], [StateDegeri],[idA3masterReference] FROM {catalogValue}.Ent_EPMDocState_CANCELLED WHERE [StateDegeri] = 'CANCELLED'";
						}
					}
					else
					{
        ...
        AND versionLevelA2versionInfo = (
            ...
            WHERE idA3masterReference = @idA3masterReference2
						...
						if (sourceApi.Contains("CADDocumentMgmt"))
						{
							var responseCreator = JsonConvert.DeserializeObject<WTUsers>(jsonWTUSER);
							EmailControlString(responseCreator);
							//Zaman kontrolünü buraya ekleyebilriiz bellir bir saat aralýðýnda diðerleri gibi burasýda tetik alýr farklý tablo üzerinden
									...
									if (cadReferencesJSON != null)
									{
										try
										{
											...
											if (string.IsNullOrEmpty(CADReferencesResponse_ID))
											{
												...
												logService.CreateJsonFileLogError(jsonData4, "Released iþlemi gerçekleþtirildi ama Drawing Reference bulunamadý");
											}
											else
											{
...
WHERE [idA2A2] = '{resolvedItems_SQL_EPMDocument.idA3masterReference}'";
										...
										catch (NullReferenceException ex)
										{
											LogService logService = new LogService(_configuration);
											var jsonData4 = JsonConvert.SerializeObject(CADResponse);
											logService.CreateJsonFileLogError(jsonData4, "CADReferencesResponse nesnesi null. Hata detayý: " + ex.Message);
										}
										catch (Exception ex)
										{
											LogService logService = new LogService(_configuration);
											logService.CreateJsonFileLogError("Beklenmedik bir hata oluþtu. Hata detayý: " + ex.Message);
										}
									}
															...
															if (CADResponse.Attachments != null && CADResponse.Attachments.Count > 0)
															{
																...
																if (CADResponse.Attachments.Any(x => x.Content.Label.Contains(CADResponse.Number + '_' + CADResponse.Version)))
																{
																	...
																	else
																	{
																		...
																		var jsonData4 = JsonConvert.SerializeObject(CADResponse);
																		//logService.CreateJsonFileLog(jsonData4, "Attachment da Veri bulunamadý.");
																		logService.CreateJsonFileLogError(jsonData4, $"Released iþlemi gerçekleþtirildi ama Attachment da {CADResponse.Number}_{CADResponse.Version}'ý içeren PDF bulunmadý.");
																...
																else
																{
																	...
																	var jsonData4 = JsonConvert.SerializeObject(CADResponse);
																	//logService.CreateJsonFileLog(jsonData4, "Attachment da Veri bulunamadý.");
																	logService.CreateJsonFileLogError(jsonData4, $"Released iþlemi gerçekleþtirildi ama Attachment da {CADResponse.Number}'ý içeren Attachment bulunmadý.");
...
WHERE [idA2A2] = '{resolvedItems_SQL_EPMDocument.idA3masterReference}'";
                                            ...
                                            WHERE EPMDocID = @Ids", new { Ids = partItem.EPMDocID });
																...
																if (now.Hour >= 20 && now.Hour < 23 && now.Minute <= 59)
																{
																	...
																	await conn.ExecuteAsync($@"
                                        DELETE FROM [{catalogValue}].[Ent_EPMDocState_CANCELLED_ERROR]
                                        WHERE EPMDocID = @Ids", new { Ids = partItem.EPMDocID });
                                        ...
                                        WHERE EPMDocID = @Ids", new { Ids = partItem.EPMDocID });
															...
															new { EPMDocID = Ent_EPMDocStateModelResponse.EPMDocID, idA3masterReference = Ent_EPMDocStateModelResponse.idA3masterReference }
																	...
																	{
																		// Veri yoksa yeni bir kayýt ekle
																		await conn.ExecuteAsync(
...
new
{
							...
							if (!string.IsNullOrEmpty(partCode))
							{
								...
								if (partState == "RELEASED")
								{
									...
									try
									{
										...
										{
											var wtAttachResponse = await WTDocumentAttachmentFunc(CADViewResponseContentInfo.content, CADViewResponseContentInfo.name, CADResponse.ID);

										}
											var dataResponse = await _apiService.PostDataAsync(apiFullUrl, apiURL, endPoint, LogJsonData, LogJsonData);
	...
	new { EPMDocID = Ent_EPMDocStateModelResponse.EPMDocID, idA3masterReference = Ent_EPMDocStateModelResponse.idA3masterReference }
										...
										//                   await conn.ExecuteAsync(
										//$"INSERT INTO [{catalogValue}].[Ent_EPMDocState_ERROR] ([EPMDocID],[StateDegeri], [idA3masterReference], [CadName],[name], [docNumber]) VALUES (@EPMDocID,@StateDegeri, @idA3masterReference, @CadName,@name, @docNumber)",
										//new { EPMDocID = Ent_EPMDocStateModelResponse.EPMDocID, StateDegeri = Ent_EPMDocStateModelResponse.StateDegeri, idA3masterReference = Ent_EPMDocStateModelResponse.idA3masterReference, CadName = Ent_EPMDocStateModelResponse.CadName, name = Ent_EPMDocStateModelResponse.CadName, docNumber = Ent_EPMDocStateModelResponse.docNumber });
		...
		private async Task<string> WTDocumentAttachmentFunc(string fileContentBase64, string AttachFileName, string CADResponseID)
		{
			...
			try
			{
				...
				var replicaUrlDataQuery = await _windchillApiService.WTDoc_ReplicaUrlPostData2(UploadStage1ActionResponse.Value.FirstOrDefault().ReplicaUrl, UploadStage1ActionResponse.Value.FirstOrDefault().FileNames.FirstOrDefault(), UploadStage1ActionResponse.Value.FirstOrDefault().StreamIds.FirstOrDefault(), WindchillServerName, BasicUsername, BasicPassword, token, fileContentBase64, AttachFileName, UploadStage1ActionResponse.Value.FirstOrDefault().MasterUrl);
				...
				var contentInfoList = new List<ContentInfoStage3>
		{
			new ContentInfoStage3
			{
				...
				// RootObject nesnesini oluþturun
				var rootObject = new ContentInfoStage3RootObject
				{
					ContentInfo = contentInfoList
				};

				// RootObject nesnesini JSON formatýna dönüþtürün
				...
				var SQL_ApplicationDataQuery = $"SELECT * FROM {catalogValue}.ApplicationData WHERE role = 'SECONDARY' AND [fileName] = '{AttachFileName}'";
				var SQL_ApplicationDataQueryResponse = await conn.QuerySingleAsync<dynamic>(SQL_ApplicationDataQuery);

				////Bunu Sonradan ekledim amaç ayný pdf attache eklenmesin direkt güncellesi
				//var SQL_ApplicationDataQuery = $"SELECT * FROM {catalogValue}.ApplicationData WHERE role = 'SECONDARY' AND [fileName] = '{AttachFileName}'";
				//var SQL_ApplicationDataQueryResponse = await conn.QuerySingleAsync<dynamic>(SQL_ApplicationDataQuery);
				...
				await conn.ExecuteAsync(
					...
					$"WHERE [idA3B5] = '{UploadStage3ActionResponse.Value.FirstOrDefault().ID.Split(':')[2]}'");
